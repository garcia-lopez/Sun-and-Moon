function darkMode() {
    ...
    const animations = applyAnimationBasedOnBreakpoint();

    if (moon.classList.contains('sun')) {
        moon.style.animation = 'DarkNight 1s linear';
        centro.style.animation = animations[0];
       body_background.style.backgroundImage = 'radial-gradient(circle at center, #2B124C 60%, #010119)';
        ...
        centro.style.animation = animations[1];
...
function applyAnimationBasedOnBreakpoint() {
    const viewportWidth = window.innerWidth;
    let animations = [];

    if (viewportWidth <= 600){
        //small screens
        animations = ['DarkNightCenterSmall 0.5s linear', 'SunnyDayCenterSmall 0.5s linear'];
    } else {
        animations = ['DarkNightCenter 0.5s linear', 'SunnyDayCenter 0.5s linear'];
    }
